					...
					<tr>
						...
						<td>
							<a href="../san-pham/<?= @$sanpham[0]['alias_vn'] ?>.html" target="_blank" title="">
								<?= $sanpham[0]['ten_vn'] ?></a>
							<div>(<?= $items[$i]['idframe'] ?>)</div>
							<div class="file">
								<?php
								$arrayImg = explode(',', $items[$i]['fileImg']);

								for ($j = 0; $j < count($arrayImg); $j++) {

								?>
									<a data-fancybox="images" href="<?= URLPATH . 'img_data/icon/' . $arrayImg[$j] ?>" style="font-size:12px;display:block">- <?= $arrayImg[$j] ?></a>
								<?php } ?>
							</div>
						</td>
						<td>
						...
						<td>
							<?= $d->dolla_vnd($items[$i]['price']) ?>
							<div>Frame: <?= $d->dolla_vnd($items[$i]['frame']) ?></div>
						</td>
						...
						<td>
							<font style="color:blue;font-weight:bold;"><?= $d->dolla_vnd($thanhtien) ?></font>
						</td>
					</tr>
				<?php } ?>
				</tbody>
