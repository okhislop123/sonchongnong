                ...
                if($id_don = $d->insert($data)){
                    ...
                    foreach($_SESSION['cart'] as $key => $value) {
                        ...
                        if(!empty($product)){
                            ...
                            $tongtien = $tongtien + ($price*$value['so_luong'] + $frame);
                            ...
                            $data_2['frame'] = $value['frame'];
                            $data_2['idframe'] = $value['idframe'];
                            $data_2['price'] = $value['price'];
                            $data_2['fileImg'] = $value['fileImg'];
...
<div class="container">
    ...
    <div class="row">
        <div class="col-md-4" >
            <h3 class="title-home" style="font-size: 20px;border: none;text-align: center;margin-bottom: 15px;"><?=($lang == 'vn') ? 'Order information' : 'order information';?></h3>
                    ...
                    <form action="" id="form-shopping" method="post" >
                        <div class="row m-5">
                            <div class="form-group col-sm-12 p5">
                                <input value="<?=$tenus?>" required placeholder="<?=($lang == 'vn')? 'Enter your full name (*)':'Enter your full name (*)'?>" type="text" class="form-control" id="ten" name="ten" data-error="<?=_typehoten?>">
                            </div> 
                            <div class="form-group col-sm-6 p5">
                                <input value="<?= $mailus?>" placeholder="<?=($lang == 'vn')? 'Enter email (if applicable)':'Enter email (if applicable)'?>" type="email" class="form-control" id="email" name="email" >
                            </div>
                            <div class="form-group col-sm-6 p5">
                                <input value="<?=$sdtus?>" placeholder="<?=($lang == 'vn')? 'Enter your phone number(*)':'Enter your phone number(*)'?>" required type="text" class="form-control" id="dienthoai" name="dienthoai" data-error="<?=_typesodienthoai?>">
                            </div>
                        </div>
                     
                        <div class="form-group">
                            <input value="<?= $diachius?>" type="text" placeholder="<?=($lang == 'vn')? 'Enter your shipping address':'Enter your shipping address'?>" required class="form-control" id="diachi" name="diachi" data-error="<?=_typeaddress?>">
                        </div>
                        
                        <input value="0" type="hidden" id="inp_phiship" name="phi_ship">

                        <div class="form-group">
                            <textarea id="loinhan"  placeholder="<?=($lang == 'vn')? 'Enter your message':'Enter your message'?>" class="form-control" rows="3" name="loinhan"></textarea>
                        </div>

                        <div class="form-group">
                            <label for="thanhtoan"><?=($lang == 'vn')? 'Select a payment method':'Select a payment method'?></label>
                            <select name="thanhtoan" class="form-control" id="thanhtoan">
                                ...
                                ?>
                                <option value="<?=$httt['id'] ?>" <?php if(!empty($_POST['thanhtoan']) && $_POST['thanhtoan'] == $httt['id'] ) echo 'selected="selected"' ?>><?=$httt['ten_us'] ?></option>
                                <?php } ?>
                            </select>
                        </div>
                        <div class="form-group">
                        ...
                        <div class="form-group">                        
                            <button type="submit" class="btn btn-login" name="guidonhang">Order</button>
        ...
        <div class="col-md-8">
            <div class="page-cart">
                <div class="info-cart">
                    <div class="">
                        <h3 class="title-home" style="font-size: 20px;margin-bottom:15px;border: none;text-align: center;margin-bottom: 15px;"><?=($lang == 'vn')? 'Order details':'Order details'?></h3>
                    </div>
                            ...
                            <table class="table table-hover table-bordered ">
                                <tbody>
                                    <tr>
                                        ...
                                        <th style="width:25%;" class="">Product Name</th>
                                        <th style="width:15%; text-align: center;">Price</th>
        
                                        <th style="width:10%;" align="center" class="th_soluong">Amount</th>
                                        <th style="width:15%;" >Provisional</th>
                                        <th style="width:10%;" >
                                            Delete
                                        </th>
                                    </tr>
                                    <?php
                                        ...
                                        if(count($_SESSION['cart'])>0){ 
                                            ...
                                            if(!empty($product)){
                                                $id_product = $product['id'];
                                                $price = $value['price'];
                                                $tongtien = $tongtien + ($price*$value['so_luong']) + $value['frame'];
                                            $stt++;
                                            ?>
                                            <tr>
                                                <td><?=$stt?></td>
                                                ...
                                                <td>
                                                    <a href="<?=URLPATH.$product['alias_'.$_SESSION['lang']] ?>.html" >
                                                        ...
                                                        <div>(<?=$value['idframe']?>)</div>
                                                        <div class="file">
                                                            <?php 
                                                                $arrayImg = explode(',',$value['fileImg']);
                                                                
                                                                for($i = 0;$i< count($arrayImg);$i++){

                                                            ?>
                                                                <a  data-fancybox="images" href="<?=URLPATH.'img_data/icon/'.$arrayImg[$i]?>" style="font-size:12px;display:block" >- <?=$arrayImg[$i]?></a>
                                                            <?php } ?>
                                                        </div>
                                                    </a>
                                                </td>
                                                ...
                                                <td align="left">
                                                    ...
                                                    <?php
                                                        echo $d->dolla_vnd($price*$value['so_luong'] + $value['frame']);
                                                    ?>
                                                    </div>
                                                </td>
                                                 <td align="left">
                                                    <a href="javascript:;" onclick="xoa_sp_gh_dh('<?=$product['id'] ?>','<?=$_SESSION['iddonhang'] ?>','<?=_redel?>?')" title="Xóa sản phẩm"><i class="fas fa-trash-alt"></i></a>
                                                </td>
                                            </tr>
                                    <?php }}} ?>
                                    <tr>
                                        <td colspan="5"  class="text-right"><?=($lang == 'vn')? 'Total amount:':'Total amount:'?></td>
                                        <td class="text-right"><font class="color-main"><?=$d->dolla_vnd($tongtien)?></font></td>
                                    </tr>
                                    <!--tr>
                                    ...
                                    <div class="mua-tiep">
                                        <a href="<?=URLPATH?>" style="color: red;"><?=($lang == 'vn')? 'Continue buying':'Continue buying'?></a>
                                    </div>
                                    </td>
                                    <td colspan="4" style="border-left: 0;">
                                        <div class="tong_tt">
                                        <h3 class="text-right"><span><?=($lang == 'vn')? 'Amount to pay:':'Amount to pay:'?></span> <font id="tong_tien_gh" class="color-main"><?=$d->dolla_vnd($tongtien)?></font></h3>
                                        </div>
                                    </td>
